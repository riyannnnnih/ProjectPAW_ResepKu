...
class EditProfileModel {
    ...
    {
        $select = mysqli_query($this->db, "SELECT * FROM `pengguna` WHERE id_pengguna = '$user_id'") or die('query failed');
    ...
    public function updateProfile($user_id, $updateData)
    {
        ...
        if(!empty($update_pass) || !empty($new_pass) || !empty($confirm_pass)){
            if($update_pass != $old_pass){
                ...
                mysqli_query($db, "UPDATE `pengguna` SET password = '$confirm_pass' WHERE id_pengguna = '$user_id'") or die('query failed');
        ...
        if(!empty($update_image)){
            if($update_image_size > 2000000){
                ...
                $image_update_query = mysqli_query($db, "UPDATE `pengguna` SET image = '$update_image' WHERE id_pengguna = '$user_id'") or die('query failed');
